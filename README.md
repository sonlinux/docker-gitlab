...
- Alison Mukoma ([sonlinux](https://github.com/sonlinux))
- ...
# Prerequisites
...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/alisonmukoma/gitlab) and is the recommended method of installation.

```bash
docker pull alisonmukoma/gitlab:14.3.1
```

You can also pull the `latest` tag which is built from the repository *HEAD*

```bash
docker pull alisonmukoma/gitlab:latest
```

Alternatively you can build the image locally.

```bash
docker build -t alisonmukoma/gitlab github.com/alisonmukoma/docker-gitlab
```
...
```bash
wget https://raw.githubusercontent.com/sonlinux/docker-gitlab/master/docker-compose.yml
```
...
To illustrate linking with a postgresql container, we will use the [alisonmukoma/postgresql](https://github.com/alisonmukoma/docker-postgresql) image. When using postgresql image in production you should mount a volume for the postgresql data store. Please refer the [README](https://github.com/alisonmukoma/docker-postgresql/blob/master/README.md) of docker-postgresql for details.
...
Here the image will also automatically fetch the `DB_NAME`, `DB_USER` and `DB_PASS` variables from the postgresql container as they are specified in the `docker run` command for the postgresql container. This is made possible using the magic of docker links and works with the following images:
 ...
 - [alisonmukoma/postgresql](https://quay.io/repository/alisonmukoma/postgresql/)
...
If you are using PostgreSQL image other than [alisonmukoma/postgresql](https://quay.io/repository/alisonmukoma/postgresql/) you will need make sure that the image you are using can handle migration itself, **or**, you will need to migrate the data yourself before starting newer version of PostgreSQL.
...
Copy the `ca.crt` file into the certs directory on the [datastore](#data-store). The `ca.crt` file should contain the root certificates of all the servers you want to trust. With respect to GitLab CI, this will be the contents of the gitlab_ci.crt file as described in the [README](https://github.com/alisonmukoma/docker-gitlab-ci/blob/master/README.md#ssl) of the [docker-gitlab-ci](https://github.com/alisonmukoma/docker-gitlab-ci) container.
...
If you are using the [docker-redmine](https://github.com/alisonmukoma/docker-redmine) image, you can *one up* the gitlab integration with redmine by adding `--volumes-from=gitlab` flag to the docker run command while starting the redmine container.
...
docker run --name gitlab -d [OPTIONS] \
    alisonmukoma/gitlab:14.3.1 app:sanitize
```
...
| `NGINX_HSTS_ENABLED` | Advanced configuration option for turning off the HSTS configuration. Applicable only when SSL is in use. Defaults to `true`. See [#138](https://github.com/alisonmukoma/docker-gitlab/issues/138) for use case scenario. |
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    alisonmukoma/gitlab:14.3.1 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    alisonmukoma/gitlab:14.3.1 app:rake db:setup
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    alisonmukoma/gitlab:14.3.1 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    alisonmukoma/gitlab:14.3.1 app:rake gitlab:backup:restore BACKUP=1515629493_2020_12_06_13.0.6
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    alisonmukoma/gitlab:14.3.1 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    alisonmukoma/gitlab:14.3.1 app:rake gitlab:import:repos
```
...
> **Note**
>
> Upgrading to `alisonmukoma/gitlab:14.3.1` from `alisonmukoma/gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `alisonmukoma/gitlab:8.0.5-1` before upgrading to `alisonmukoma/gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    alisonmukoma/gitlab:x.x.x app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -d [OPTIONS] alisonmukoma/gitlab:14.3.1
```
...
services:
  gitlab:
    image: alisonmukoma/gitlab:14.3.1
    healthcheck:
